...
import com.msempleados.impl.EmpleadosImplement;
...
public class EmpleadoController {
	...
	{
		Empleados emple=service.guardar(request);
		return new ResponseEntity<Empleados>(emple,HttpStatus.OK);
		
	}
    @PutMapping
    public ResponseEntity<Empleados> actualizar(@RequestBody EmpleadosRequest request)
	{
    	Empleados emple=service.actualizar(request);
    	return new ResponseEntity<Empleados>(emple,HttpStatus.OK);
	}
    @GetMapping("buscar/{id}")
    public ResponseEntity<Empleados> buscar(@PathVariable int id)
    {
    	Empleados emple = service.buscar(id);
    	return new ResponseEntity<Empleados>(emple,HttpStatus.OK);
    }
